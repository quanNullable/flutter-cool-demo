...
const int ANIM_DURATION = 500, ANIM_ADD_REMOVE_DELAY = 100, ANIM_ADD_REMOVE_DURATION = 200;
...
class AnimHelper {
  ...
  void _initAnimation() {
    _animationController = AnimationController(
        ...
        duration: controller.animDuration ?? const Duration(milliseconds: ANIM_DURATION));
  ...
  void _initAddRemoveAnimation() {
    ...
    for (int i = 0; i < controller.itemCount; i++) {
      AnimationController animationController = AnimationController(
          vsync: _tickerProvider, duration: const Duration(milliseconds: ANIM_ADD_REMOVE_DURATION));
  ...
  void _addTransform(int position, double width, double height) {
    ...
    cardItem.transformWidget = controller.transformAdd(cardItem.widget, animation.value,
        _getCurveValue(animation.value), width, height, position + 1, position);
  }
  ...
  void _removeTransform(int position, double width, double height) {
    ...
    cardItem.transformWidget = controller.transformRemove(cardItem.widget, animation.value,
        _getCurveValue(animation.value), width, height, position, position);
  }
  ...
  void _commonTransform(double width, double height, int fromPosition, int toPosition) {
    ...
    controller.zIndexTransformCommon(cardItem, _animation.value, _getCurveValue(_animation.value),
        width, height, fromPosition, toPosition);
    cardItem.transformWidget = controller.transformCommon(cardItem.widget, _animation.value,
        _getCurveValue(_animation.value), width, height, fromPosition, toPosition);
  }
  ...
  void _toFrontTransform(double width, double height, int fromPosition, int toPosition) {
    CardItem cardItem = _cardList[fromPosition];
    controller.zIndexTransformToFront(cardItem, _animation.value, _getCurveValue(_animation.value),
        width, height, fromPosition, toPosition);
    cardItem.transformWidget = controller.transformToFront(cardItem.widget, _animation.value,
        _getCurveValue(_animation.value), width, height, fromPosition, toPosition);
  }
  ...
  void _toBackTransform(double width, double height, int fromPosition, int toPosition) {
    CardItem cardItem = _cardList[fromPosition];
    controller.zIndexTransformToBack(cardItem, _animation.value, _getCurveValue(_animation.value),
        width, height, fromPosition, toPosition);
    cardItem.transformWidget = controller.transformToBack(cardItem.widget, _animation.value,
        _getCurveValue(_animation.value), width, height, fromPosition, toPosition);
  }
