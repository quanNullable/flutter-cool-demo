...
class ExplosionWidget extends StatefulWidget {
  ...
  const ExplosionWidget({Key key, this.child, this.bound, this.tag}) : super(key: key);
...
class ExplosionRenderObjectWidget extends RepaintBoundary {
  ...
  const ExplosionRenderObjectWidget(
      {Key key, Widget child, this.byteData, this.imageSize, this.progress, this.bound})
      : super(key: key, child: child);

  @override
  _ExplosionRenderObject createRenderObject(BuildContext context) =>
      _ExplosionRenderObject(byteData: byteData, imageSize: imageSize, bound: bound);

  @override
  void updateRenderObject(BuildContext context, _ExplosionRenderObject renderObject) {
...
class _ExplosionRenderObject extends RenderRepaintBoundary {
  ...
  _ExplosionRenderObject({this.byteData, this.imageSize, this.bound, RenderBox child})
      : super(child: child);
  ...
  void paint(PaintingContext context, Offset offset) {
    if (byteData != null && imageSize != null && progress != 0 && progress != 1) {
      if (particles == null) {
        if (bound == null) {
  ...
  for (int i = 0; i < partLen; i++) {
    for (int j = 0; j < partLen; j++) {
      particles[(i * partLen) + j] = generateParticle(
          getColorByPixel(
              byteData, imageSize, Offset((j + 1) * w.toDouble(), (i + 1) * h.toDouble())),
          random,
          bound);
    }
  }
...
_Particle generateParticle(Color color, Math.Random random, Rect bound) {
  ...
  particle.top =
      nextDouble < 0.2 ? particle.top : particle.top + ((particle.top * 0.2) * random.nextDouble());
